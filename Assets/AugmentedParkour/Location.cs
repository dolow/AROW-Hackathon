...
public class Location : MonoBehaviour
{
    ...
    {
#if UNITY_EDITOR
        Debug.Log("UnityEditorでは確認できません。");
#else

        if (_locationManager.Disabled)
        {
            debug.Debug.Log("GPS機能を有効にすると確認できます。");
        }
        else
        {
            Debug.Log("位置情報が有効か: " + _locationManager.Started.ToString()
        ...
        if (Physics.Raycast(origin, Vector3.down, out hitInfo) && flag)
        {
            Debug.Log(origin);
